...
abstract: |
  ...
  We then evaluate two orthogonal interventions on a smaller YOLOv8n detector trained on the same Tier-3 disease pool. The first measures how the model behaves under simulated image degradation. The clean-trained model is fairly stable under blur and motion blur but loses most of its mAP\@0.5 under Gaussian noise (0.360 to 0.078). Training on a clean-plus-degraded mixture closes that gap and lifts performance on every degraded test condition, with the largest gain on noise (0.078 to 0.365). The second compares three input-preprocessing scales on the same detector. Both region-focused settings outperform the full-image input on the test set (Dental ROI 0.405 mAP\@0.5, Quadrants 0.402, Full 0.360), with the dental-region crop best by a small margin.
  Together, these results suggest that on datasets of this scale, additional weakly-related supervision does not automatically improve fine-grained detection. Class imbalance and the intrinsic difficulty of particular diseases are larger limits than the total number of labeled boxes, and image-quality augmentation and region-focused preprocessing both move the needle further than the curriculum does.
format:
  pdf:
...
This paper addresses those concerns directly. We train a three-stage curriculum YOLOv8m segmentation model on the DENTEX 2023 release and compare it, under matched hyperparameters, with a single-stage baseline trained only on the fully labeled tier. We also examine the checkpoints produced at each stage of the curriculum to isolate which stage contributes the most to the final model. A parallel track of the project evaluates how the resulting model behaves under realistic image degradation and under region-focused preprocessing of the panoramic image.
...
All runs start from ImageNet-pretrained weights in the YOLOv8 family (Jocher et al. 2023). The curriculum and baseline runs for RQ1 and RQ2 use YOLOv8m-seg, the medium segmentation variant, which produces bounding boxes and polygon masks in one forward pass and matches the DENTEX test-set annotation format. RQ3 and RQ4 use YOLOv8n, the nano detection-only variant, which is sufficient for their internal comparisons and keeps training cost manageable.
...
## RQ4. Region-Focused Evaluation

To address the question: To what extent does region-focused preprocessing improve YOLOv8 disease detection performance compared with using the full panoramic image? We found during the EDA, the disease annotations were concentrated around the dental arches instead of being spread across the whole image. Therefore, we tested whether removing unused background with ROI crops, or zooming further into quadrant crops, could help the detector focus on smaller disease regions.

### Cropped Image

Before comparing model scores, we first checked the cropped images visually. This step was important because the ROI and quadrant datasets were newly created from the original Stage 3 images. The goal was to make sure the crops still kept the dental regions and that the transformed YOLO labels still matched the correct tooth areas.

![Original panoramic X-ray](results/figures/original.png){#fig-rq4-original width=85%}

![Dental ROI crop](results/figures/roi.png){#fig-rq4-roi width=85%}

| Upper-left quadrant | Upper-right quadrant |
|---|---|
| ![](results/figures/quad_1.png){width=100%} | ![](results/figures/quad_2.png){width=100%} |
| Lower-left quadrant | Lower-right quadrant |
| ![](results/figures/quad_3.png){width=100%} | ![](results/figures/quad_4.png){width=100%} |

Figure @fig-rq4-original and Figure @fig-rq4-roi show how the same X-ray changes under the original and ROI input formats. The original image keeps the full panoramic view, including large non-dental background areas. The ROI crop removes much of this outside region while keeping the main upper and lower dental arches. The quadrant crops split the ROI into smaller regions, so each crop gives the model a closer view of one part of the mouth. This visual check confirms that the preprocessing keeps the important dental structures while changing the scale and amount of background seen by the model.

### Region-Focused Model Comparison

After creating the ROI and quadrant datasets, we trained three different YOLOv8 models. First, the model used the original Stage 3 panoramic images and served as the baseline. Second, the ROI model used the dental ROI crops. Lastly, the quadrant model used the four-quadrant crops. All three models were evaluated on their matching test sets.

@tbl-rq4-region-results summarizes the test results for three different models.

| Model | mAP\@0.5 | mAP\@0.5-0.95 | mAP\@0.5 difference from full | mAP\@0.5-0.95 difference from full |
|---|---:|---:|---:|---:|
| Full image | 0.360 | 0.232 | 0.000 | 0.000 |
| Dental ROI | 0.405 | 0.268 | 0.046 | 0.035 |
| Quadrants | 0.402 | 0.274 | 0.042 | 0.042 |

: Region-focused YOLOv8 test results across full-image, ROI, and quadrant inputs. {#tbl-rq4-region-results}

As we can see from the @tbl-rq4-region-results, the full-image model reached mAP\@0.5 of 0.360 and mAP\@0.5-0.95 of 0.232. The ROI model improved to 0.405 on mAP\@0.5 and 0.268 on mAP\@0.5-0.95. The quadrant model reached 0.402 on mAP\@0.5 and 0.274 on mAP\@0.5-0.95. Compared with the full-image baseline, the ROI model improved mAP\@0.5 by 0.046, while the quadrant model improved it by 0.042.

![Region-Focused YOLOv8 Comparison](results/figures/rq4_comparison.png){#fig-rq4-region-bar width=85%}

Figure @fig-rq4-region-bar shows the same comparison visually for mAP\@0.5. Both region-focused settings perform better than the full-image baseline. The ROI model gives the highest mAP\@0.5, while the quadrant model gives the highest mAP\@0.5-0.95. Overall, these results suggest that region-focused preprocessing improves YOLOv8's ability to detect dental disease. However, the ROI and quadrant models are very similar, so splitting the image into four quadrants does not offer a clear advantage over a single ROI crop.
...
The region-focused results show that cropping helped disease detection. Both the ROI and quadrant models performed better than the full-image model, which matches the EDA result that annotations were concentrated around the dental arches. The ROI model had the best mAP\@0.5, while the quadrant model had the best mAP\@0.5-0.95, but the two results are very close. This suggests that removing background is useful, but splitting the mouth into four smaller crops does not add a clear advantage over the ROI crop.

# Limitations

There exist a class imbalance in the Stage 3 disease labels. Caries makes up most of the training annotations, while Periapical Lesion is much rarer. Because the Stage 3 set only has about 700 training images, simple balancing methods were hard to use. Undersampling would remove too much data, while oversampling did not fully solve the rare-class problem. We also could not easily increase the training set size because dental X-rays are carefully labeled medical images, and they involve real patient data. Privacy concerns and annotation cost make it hard to collect more comparable data.
...
For the region-focused experiment, the ROI and quadrant crop rules were fixed using simple image proportions rather than being learned from each X-ray. This makes the method easy to reproduce, but it may not fit every patient image equally well. A few disease labels were also lost after cropping. For the quadrant setting, many crop label files were empty because each image was split into four parts, and not every quadrant contained a disease annotation. These empty crops are valid negative examples, but they also make the quadrant dataset different from the full-image dataset.
...
Finally, the region-focused preprocessing experiment shows that where the model looks also matters. Both the ROI and quadrant models performed better than the full panoramic image model. This supports the EDA finding that disease annotations are concentrated around the dental arches, while much of the full X-ray contains background. The ROI model gave the best mAP\@0.5, and the quadrant model gave the best mAP\@0.5-0.95, but the two results were very close. Therefore, cropping to the dental region appears useful, while splitting the image into quadrants does not clearly improve beyond the ROI crop.

Overall, the results suggest that simply adding more off-target supervision is not enough for this dataset. More direct changes, such as degradation-based augmentation and region-focused preprocessing, are more useful because they address the actual weaknesses of the disease-detection task.

# LLM Usage

We used ChatGPT-5 as a writing and coding support tool during this project. It was used to help debug Python and Quarto issues, organize files and notebook sections, summarize intermediate results, and improve formatting. All final experiment design decisions, code execution, result interpretation, and report revisions were reviewed and completed by the authors.
